...
async def chat(
    ...
    """RAG chat â€” retrieve context then answer with Ollama."""
    ...
    # 2. Retrieve
    hits = store.search(query_vector=query_vec, top_k=request.top_k, doc_ids=request.doc_ids or None)
    if not hits:
        raise HTTPException(
...
async def chat_stream(
    ...
    """Streaming RAG â€” returns SSE token stream."""
    query_vec = await embedder.embed(request.query)
    hits = store.search(query_vector=query_vec, top_k=request.top_k, doc_ids=request.doc_ids or None)
    if not hits:
